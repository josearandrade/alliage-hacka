...
def image_to_tensor(image: Image.Image, backbone_key: str, image_size=IMAGE_SIZE) -> torch.Tensor:
...
def extract_batch(model, images: list[Image.Image], device: str, backbone_key: str) -> tuple[np.ndarray, np.ndarray]:
    size = getattr(model, "_alliage_image_size", IMAGE_SIZE)
    batch = torch.stack([image_to_tensor(image, backbone_key, size) for image in images]).to(device)
    with torch.autocast(device_type="cuda", enabled=device == "cuda"):
        tokens = model(pixel_values=batch).last_hidden_state
    tokens = tokens.float().cpu().numpy()
    patches = tokens[:, 1:, :]
    rows, cols = size[1] // 14, size[0] // 14
    grid = patches.reshape(len(images), rows, cols, -1)
    boundaries = np.linspace(0, cols, 4, dtype=int)
    regions = [
        grid[:, y0:y1, boundaries[col]:boundaries[col + 1]].mean(axis=(1, 2))
        for y0, y1 in ((0, rows // 2), (rows // 2, rows))
        for col in range(3)
    ]
